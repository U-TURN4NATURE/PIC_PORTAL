    ...
    <div className="space-y-8">
      ...
      <div className="glass-card rounded-2xl p-8 relative overflow-hidden bg-gradient-to-r from-brand-forest/90 to-brand-olive border-none">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl font-dm-serif text-white mb-2">Welcome back, {user?.fullName}! 🌱</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-2xl p-6 border border-brand-sage/30 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(wallet.availableBalance || 0)}</h3>
            </div>
            <div className="p-3 bg-brand-forest/10 rounded-xl text-brand-forest">
              <Wallet className="w-5 h-5" />
            </div>
          </div>
          <Link href="/dashboard/wallet" className="text-sm text-brand-forest font-medium flex items-center hover:underline">
        ...
        <div className="bg-white rounded-2xl p-6 border border-brand-sage/30 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(wallet.totalEarnings || 0)}</h3>
            </div>
            <div className="p-3 bg-brand-gold/20 rounded-xl text-yellow-700">
              <IndianRupee className="w-5 h-5" />
            </div>
          </div>
          <p className="text-xs text-gray-500">Lifetime commission earned</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-brand-sage/30 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(wallet.pendingEarnings || 0)}</h3>
            </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-brand-sage/30 shadow-sm hover:shadow-md transition-shadow">
          ...
          <Link href="/dashboard/orders" className="text-sm text-brand-forest font-medium flex items-center hover:underline">
      ...
      <div className="bg-white border border-brand-sage/30 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-brand-sage/20 flex justify-between items-center">
          ...
          <Link href="/dashboard/orders" className="text-sm font-medium text-brand-forest hover:underline">
            View All
          </Link>
        </div>
              ...
              {recentOrders.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-gray-500">
                    No orders referred yet. Share your link to start earning!
                  </td>
                </tr>
              ) : (
